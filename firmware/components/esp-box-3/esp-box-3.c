...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    /* Legacy I2C driver (v1) requires scl_speed_hz == 0 because bus clock was
       already configured during bsp_i2c_init(). Newer esp_lcd_touch macros
       set scl_speed_hz=100000 for the v2 driver, which causes v1 to return
       ESP_ERR_INVALID_ARG ("scl_speed_hz is not need to set in legacy i2c_lcd driver"). */
...
{
    /* Non-fatal touch init: on failure, log and continue (boot the display). */
    ...
    ESP_LOGI(TAG, "Touch input device registered successfully with LVGL");
    return disp_indev;
}
...
lv_disp_t *bsp_display_start_with_config(const bsp_display_cfg_t *cfg)
{
    ...
    /* Touch/indev is optional: failing to add one must not abort the display. */
    (void)bsp_display_indev_init(disp);
...
static uint8_t bsp_get_main_button(void *param)
{
    /* Touch panel unavailable -> MAIN (home) button never pressed. */
    if (tp == NULL) {
        return false;
    }
...
static esp_err_t bsp_init_main_button(void *param)
{
    /* Touch-based "home" button: if the touch panel isn't available (e.g.
       i2c driver mismatch), don't abort — the MAIN button is optional. */
    if (tp == NULL) {
        esp_err_t e = bsp_touch_new(NULL, &tp);
        if (e != ESP_OK || tp == NULL) {
            ESP_LOGE(TAG, "MAIN (touch home) button unavailable; continuing.");
            return ESP_OK;
        }
    }
    return ESP_OK;
